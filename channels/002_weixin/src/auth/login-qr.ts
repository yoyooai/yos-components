...
import { apiGetFetch, apiPostFetch } from "../api/api.ts";
import { listIndexedWeixinAccountIds, loadWeixinAccount } from "./accounts.ts";
import { normalizeWeixinBaseUrl } from "./accounts.ts";
import { logger } from "../util/logger.ts";
import { redactToken } from "../util/redact.ts";
...
export type WeixinQrWaitResult = {
   ...
   * this YOS instance, so no new credentials are issued and existing
...
async function refreshQRCode(
  ...
  try {
    ...
    return { success: false, message: "刷新二维码失败，请检查网络后重试。" };
  }
}
      ...
      switch (statusResponse.status) {
        ...
        case "binded_redirect": {
          ...
          return {
            ...
            message: "已连接过此 YOS，无需重复连接。",
          };
        }
        case "scaned_but_redirect": {
          const redirectHost = statusResponse.redirect_host;
          if (redirectHost) {
            const newBaseUrl = normalizeWeixinBaseUrl(`https://${redirectHost}`);
        ...
        case "confirmed": {
          ...
          return {
            ...
            message: "已将此 YOS 连接到微信。",
          };
        }
      }

    } catch (err) {
      logger.error("Error polling QR status");
      activeLogins.delete(opts.sessionKey);
      return {
        connected: false,
