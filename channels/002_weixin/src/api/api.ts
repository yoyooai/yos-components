...
import { loadConfigBotAgent, loadConfigRouteTag } from "../auth/accounts.ts";
import { logger } from "../util/logger.ts";
import { redactBody, redactUrl } from "../util/redact.ts";
 ...
 */
const DEFAULT_BOT_AGENT = "YOS/0.1.0-alpha.1";
...
export async function apiGetFetch(params: {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`${params.label} http_${res.status}`);
    }
    return rawText;
  } catch (err) {
    if (t !== undefined) clearTimeout(t);
    ...
    logger.error(
      `${params.label}: GET fetch failed url=${redactUrl(url.toString())} timeoutMs=${timeoutMs ?? "none"} type=${classified.type} description=${classified.description}${classified.code ? ` code=${classified.code}` : ""}`,
    );
    throw err;
  }
}
...
export async function apiPostFetch(params: {
  ...
  try {
    ...
    const classified = classifyFetchError(err);
    logger.error(
      `${params.label}: POST fetch failed url=${redactUrl(url.toString())} timeoutMs=${params.timeoutMs ?? "none"} type=${classified.type} description=${classified.description}${classified.code ? ` code=${classified.code}` : ""}`,
    );
    throw err;
  } finally {
    cleanup();
 ...
 * after YOS has already aborted the long-poll.
 */
