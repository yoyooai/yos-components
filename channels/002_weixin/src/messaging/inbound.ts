...
import type { WeixinMessage, MessageItem } from "../api/types.ts";
import { MessageItemType } from "../api/types.ts";
...
function persistContextTokens(accountId: string): void {
  ...
  try {
    ...
    fs.writeFileSync(temporary, JSON.stringify(tokens, null, 0), { mode: 0o600 });
    fs.renameSync(temporary, filePath);
    fs.chmodSync(filePath, 0o600);
  } catch (err) {
    logger.warn("persistContextTokens: failed to write private state");
  }
}
...
export function restoreContextTokens(accountId: string): void {
  ...
  try {
    ...
    logger.warn("restoreContextTokens: failed to read private state");
  }
}
...
export function clearContextTokensForAccount(accountId: string): void {
  ...
  try {
    ...
    logger.warn("clearContextTokensForAccount: failed to remove private state");
  }
...
function generateMessageSid(): string {
  return generateId("yos-weixin");
}

/** Inbound context passed to the YOS C4 pipeline. */
...
export function weixinMessageToMsgContext(
  ...
  const ctx: WeixinMsgContext = {
    ...
    MessageSid: String(msg.message_id ?? msg.item_list?.[0]?.msg_id ?? generateMessageSid()),
