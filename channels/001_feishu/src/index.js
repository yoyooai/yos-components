...
// Load .env from ~/yos/.env (absolute path, not cwd-dependent)
...
if (connectionMode === 'webhook' && !config.bot?.verification_token) {
  ...
  console.error(`[feishu] Set it in ~/yos/components/feishu/config.json (get from developer console → Event Subscriptions).`);
  process.exit(1);
}
 ...
 */
function sendToC4(source, endpoint, messageId, content, onReject) {
  if (!content) {
  ...
  if (chatType === 'p2p') {
    ...
    if (imageKeys.length > 0) {
      ...
      if (mediaPaths.length > 0) {
        ...
        sendToC4('feishu', endpoint, messageId, msg, rejectReply);
      } else {
  ...
  if (chatType === 'group') {
    ...
    if (imageKeys.length > 0) {
      if (smartNoMention) {
        ...
        sendToC4('feishu', endpoint, messageId, msg, groupRejectReply);
        return;
      }
...
if (!creds.app_id || !creds.app_secret) {
  console.error('[feishu] FEISHU_APP_ID and FEISHU_APP_SECRET must be set in ~/yos/.env');
  process.exit(1);
}
