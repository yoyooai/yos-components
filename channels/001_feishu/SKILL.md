...
lifecycle:
  ...
  service:
    ...
    name: yos-feishu
    entry: src/index.js
  data_dir: ~/yos/components/feishu
  hooks:
...
Feishu communication channel for yos.
...
**Prerequisites** (installed automatically by `yos add feishu` / `yos upgrade feishu` — see `hooks/post-install-shared.js`):
...
- App credentials in lark-cli's keychain (`~/.lark-cli/config.json` + AES-256-GCM encrypted file under `~/.local/share/lark-cli/`); pushed from `~/yos/.env` automatically.
...
5. **All `auth login` invocations MUST include `--profile feishu`** to ensure credentials land in the correct named profile, especially when legacy lark channel is also installed on the same machine.
...
The `--profile feishu` flag selects this component's credentials. Always include it — without it, lark-cli uses whichever profile is active, which may belong to legacy lark channel if both are installed. The `runLarkCli()` bridge in `src/lib/lark-cli-bridge.js` injects this flag automatically.
...
cat <<'EOF' | node ~/yos/.claude/skills/comm-bridge/scripts/c4-send.js "feishu" "<chat_id>"
Hello!
EOF
...
```bash
node ~/yos/.claude/skills/feishu/scripts/send.js <chat_id> "Hello!"
```
...
```bash
ADM="node ~/yos/.claude/skills/feishu/src/admin.js"
...
After changes, restart: `pm2 restart yos-feishu`
...
```bash
# Download image
node ~/yos/.claude/skills/feishu/scripts/download.js image <message_id> <image_key>

# Download file
node ~/yos/.claude/skills/feishu/scripts/download.js file <message_id> <file_key> [filename]

# Examples:
node ~/yos/.claude/skills/feishu/scripts/download.js image om_xxx img_v3_xxx
node ~/yos/.claude/skills/feishu/scripts/download.js file om_xxx file_v3_xxx report.pdf
```
...
- Config: `~/yos/components/feishu/config.json`
- Logs: `~/yos/components/feishu/logs/`
- Media: `~/yos/components/feishu/media/`
...
Choose a connection mode in `~/yos/components/feishu/config.json`:
...
2. **Use HTTP mode**: Run `yos config set protocol http` (automatically updates Caddyfile and reloads Caddy)
...
Message logs are stored in `~/yos/components/feishu/logs/<chat_id>.log`.

## Service Management

```bash
pm2 status yos-feishu
pm2 logs yos-feishu
pm2 restart yos-feishu
```
